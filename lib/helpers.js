...
const getLargeProfileImageFromSmall = imageUrl =>
  imageUrl.replace('normal', '400x400');

const updateOrCreateProfile = profileData =>
...
const updateUserProfile = (lastTweetIdStr, tweetsUrl) =>
  fetchLastRawTweet(lastTweetIdStr, tweetsUrl)
    .then(({ data }) => updateOrCreateProfile(data))
    .catch(err => err);
