...
const fetchTweets = (options, maxId, tweets = []) =>
  fetchPage(maxId, options).then(({ dataTweets, maxTweetId }) => {
    let tweetsList = tweets.splice();
    tweetsList = uniqBy(tweets.concat(dataTweets), 'id_str'); // make sure there are no duplicate tweets

    if (tweets.length < options.maxNumberOfTweets && dataTweets.length > 1) {
      return fetchTweets(options, maxTweetId, tweets);
    }
